...
author: "Nicole Barbour and Eliezer Gurarie"
date: "`r Sys.Date()`"
output: 
...
where $s_0(\cdot) = 1 - c_0(\cdot)$ is the null probability that the animal "stays" (does not cross barrier $B$ from location $Z_t$).  This formulation has the desired properties: when $\kappa = 0$, the probability of crossing is 0, when $\kappa = 1$, the probability of crossing equals the null probability $c_0 = 1-s_0$. As $\kappa$ approaches infinity, the probability of crossing approaches 1.
...
X and Y coordinates can be extracted from the `sf` object and converted to a matrix using the `st_coordinates` function. These coordinates will form the "segments" of the barrier, which will be used with other functions to determine if they each intersect with animal movement steps or not.
...
The output includes a third column `L1` for the id of the line. If the barrier object has multiple lines (e.g., a grid or network format), this column is especially useful for keeping track of distinct geometric lines (there will be a unique number for each line). However, note that even features that *appear* to be one line are often made of multiple lines:

```{r checkuniquelines}
unique(hwy1_west_xy[,"L1"])
```

If barrier data is being provided as a non-`sf` object (matrix of barrier segments), `permeability` functions require the barrier data to contain `X` and `Y` coordinates and the third column to be named `line_id`, even if there is only 1 line in the object. The `line_id` column should contain unique numbers for each unique line.
...
Users can also annotate these coordinates with covariate values specific to each segment (see simulation examples above). Just keep in mind that for each line in your barrier coordinates matrix (e.g., each unique `line_id`), there are $n$ rows but $n - 1$ segments. 
...
More advanced options for `plotCrossings` include an interactive map (`interactive.map=TRUE`, using the `mapview` package) or a static map (`static.map=TRUE`, `ggplot2` package, with the option to include a basemap with `add.basemap=TRUE` using the `ggspatial` and `prettymapr` packages). Static maps can be easily exported as images. 
...
```{r mappingpackages2, eval = TRUE}
library(ggspatial);library(ggplot2);library(prettymapr)
```
...
The function can also handle multiple linear features; for example, if you had three highways of interest occurring in the same spatial area, you could bind them together into one large `sf` object, including a column `Feature.ID` to distinguish them. The `predictions` dataframe object should also have a column called `Feature.ID`, to map individual linear feature predictions to their own geometry.

The `mapPredictions` function additionally has options for creating 1) a static map without a basemap (default); 2) a static map *with* a basemap (using `basemaps` with an ESRI landscape map, by default - or users can select their own map service provider, map type, or map resolution, with `map.service`, `map.type`, or `map.res` - see Help file for more details); or 3) create an interactive map (with `mapview` and `interactive = TRUE`).

Users can optionally add an `sf` POINT object of their movement data to map movement tracks with the barrier(s) (using the `tracks.sf` argument) or even add their own basemap (provided to the `basemap` argument). Other optional features include providing your plotting `breaks` for $\kappa$ values and changing the map extent with `box`.

Here, we show an example predicting $\kappa$ values as a function of `km` along Highway 1 West using the fitted `spline_km_model` from above. First, a `predictions` dataframe is created, storing `km` and corresponding `barrier.id` values for each barrier segment.
...
predictions <- cbind(hwy1_west_xy, hwy1_west_z) |> 
  ...
  data.frame() |>
  subset(barrier.id %in%  # make predictions only for barrier segments in crossing table (crossings possible)
...
Note that areas of the highway spatial object where there were no potential or actual crossings are in grey.

## Gridded Barrier Example: Pronghorn from Xu et al

We perform an additional example for animals interacting with a "gridded" barrier or barrier with a network of lines using the example pronghorn and fence data from the `BaBA` package, another fantastic tool for characterizing animal behavior with respect to barriers. See [Xu et al 2021](https://besjournals.onlinelibrary.wiley.com/doi/10.1111/1365-2664.13806) for more details on the `BaBA` method and package.

The package can be installed using `devtools` with:
...
Data from the `BaBA` package for the pronghorn (n = 2) and associated fence network can be loaded with the `data` function. 
...
The `fences` object is a MULTILINESTRING currently, it's converted to a LINESTRING format.
...
The pronghorn locations can be easily transformed into spatial lines to visualize the tracks with the fence network.

```{r fig.align = "center", fig.width = 6, fig.height=6}
pronghorn_tracks <- pronghorn |>
  dplyr::group_by(Animal.ID) |>
  dplyr::summarize(do_union = FALSE) |>
  sf::st_cast("LINESTRING")

mapview(pronghorn_tracks, zcol="Animal.ID") +
  mapview(fences, color = "red")
```

The sample rate is assessed by finding the difference in time between consecutive observations for each pronghorn. The median sample rate is at 2 hours.

```{r fig.align = "center", fig.width = 6, fig.height=6}
sample.rate <- pronghorn |> 
  dplyr::group_by(Animal.ID) |>
  dplyr::mutate(dtime = c(0,round(difftime(date[-1], date[-length(date)], units ="hours")))) |>
  data.frame() |>
  ungroup()
...
The pronghorn data is further formatted for `permeability` by converting to a data frame format and adding the coordinates and the month as columns.

```{r}
pronghorn$X <- st_coordinates(pronghorn)[,1]
pronghorn$Y <- st_coordinates(pronghorn)[,2]
pronghorn_df <- pronghorn |>
  data.frame() |>
  mutate(ID = as.character(Animal.ID), Time = date) |>
  mutate(Month = format(Time, "%m")) |>
  select(ID, Time, X, Y, Month)
```

As example, the `prepPermeability` function is applied to just one individual, "103_2014".

Note that because there are so many locations, the function takes a little while to run (~ 10 min).

```{r eval = FALSE}
prong_format <- prepPermeability(move.df = subset(pronghorn_df, ID == "103_2014"),
                                 move.covar.names = "Month",
                                 barrier = fences2,
                                 sample.rate = 2)
```

```{r include = FALSE}
load("./data/prong_format.rda")
```


The `print` function gives a quick summary of the crossings. Because it's a fence "network", all 4,372 steps for this individual are in the buffer.

```{r}
print(prong_format)
```

The crossings can be easily visualized with the default options for `plotCrossings`.

```{r plotcrossingsbasic2, fig.align = "center", fig.width = 6, fig.height=6}
plotCrossings(permdata = prong_format)
```

The crossing table is built using the `permdata` object. Note that due to the high number of movement *and* barrier segments, this will take a while (~ 30 min - 1 hr). 

Run time for gridded barriers or networks such as this could be improved by simplifying the barrier, e.g., by aggregating segments (by a covariate of interest for example).

```{r ctprong, eval = FALSE}
prong_cT <- buildCrossingTable(permdata = prong_format)
```

```{r include = FALSE}
load("./data/prong_cT.rda")
```

The movement step specific covariate stored in the `permdata` object can be easily annotated to the crossing table.

```{r mergepermdata3}
track_covar <- prong_format$track[,c("ID", "Step.ID","Month")]

prong_cT <- merge(prong_cT, track_covar, by=c("ID", "Step.ID"), 
                    all.x=TRUE)

str(prong_cT)
```

The overall permeability of this fence network for this individual is low, with a $\kappa$ of 0.2 indicating that the probability of the pronghorn actually crossing the fence network is only 20% of the null probability of crossing (e.g., the fence network not being there).

```{r prongnullmodel}
null_model <- fitPermeability(data = prong_cT)
null_model
```

Monthly permeability values can also be obtained with `Month` as a covariate.

```{r prongcovarmodel}
season_model <- fitPermeability(~ Month, data = prong_cT)
season_model
```

Predictions can be made and plotted to visualize monthly differences in permeability. 

```{r predictprongcovarmodel}
predict_kappa <- data.frame(Month = unique(prong_cT$Month))
prediction <- predict(season_model, new.data = predict_kappa, se.fit = TRUE)
predict_kappa <- cbind(predict_kappa, prediction)
predict_kappa
```

```{r plotprongpredictions, fig.align = "center", fig.width = 8, fig.height=6}
ggplot(data = predict_kappa, aes(x = Month, y = kappa.hat)) +
  geom_point(size=2, position = position_dodge(width = 1)) +
  geom_errorbar(aes(ymin = ci.low, ymax = ci.high), linewidth=1, position = position_dodge(width = 1)) +
  theme_classic() + xlab(" ")+ ylab("Predicted Kappa") +
  scale_x_discrete(labels=c("Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"))
```

According to these predictions, fence permeability is highest for this one pronghorn in May and November. The monthly differences indicate a seasonal spline may be appropriate.
...
Barbour N., Gurarie E., Kelly A., and Hodson J. 2026. The permeability R package: A maximum-likelihood tool to quantify the permeability of linear barriers to animal movement. *Methods in Ecology and Evolution*.


Report any issues or bugs to the package developers:

* Nicole Barbour (Main Contact), nbarbour@towson.edu
* Eliezer Gurarie (Secondary Contact), egurarie@esf.edu
